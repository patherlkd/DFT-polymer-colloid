...
void Zero_mat(vec &v,int rows, int cols);
...
template <class T>
inline T sq(T x){return (T)(x*x);}

template <class T>
inline T cu(T x){return (T)(x*x);}

template <class T>
inline T recip(T x){return (T)1.0/((T)x);}

template <class T>
inline T frac(T x, T y){return (T)(x)/(T)y;}
...
////// ARRAY ////////////////////
dbarray::dbarray()
{setnumelems(1); elems = new db[numelems];
